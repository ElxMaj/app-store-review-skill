...
class RegistryHealthTests(unittest.TestCase):
        ...
        *,
        ...
        with tempfile.TemporaryDirectory() as directory:
            ...
            claude_manifest.write_text(
                json.dumps({"name": "app-store-review", "version": "1.2.2"}),
                encoding="utf-8",
            )
            tessl_manifest.write_text(
                json.dumps({"name": "maj-labs/app-store-review", "version": "1.2.2"}),
                encoding="utf-8",
            )
            ...
            tessl_version.write_text(
                json.dumps(
                    {
                        "links": {"self": "https://api.tessl.io/example/1.2.2"},
                        "data": {
                            "id": "version-id",
                            "type": "tile-version",
                            "attributes": {
    ...
    def test_matching_public_versions_and_scores_succeed(self):
        ...
        self.assertIn("ClaudePluginHub: 1.2.2", result.stdout)
        self.assertIn("Tessl: 1.2.2", result.stdout)
    ...
    def test_stale_claudepluginhub_version_fails_with_observed_version(self):
        ...
        self.assertIn(
            "ClaudePluginHub version mismatch: expected 1.2.2, observed 1.1.4",
            result.stderr,
        )
    ...
    def test_version_outside_json_ld_does_not_satisfy_listing_check(self):
        result = self.run_checker(
            claude_document=(
                '<div data-cache=\'{"softwareVersion":"1.2.2"}\'></div>'
    ...
    def test_version_for_another_json_ld_identity_does_not_satisfy_check(self):
        result = self.run_checker(
            claude_document=(
                '<script type="application/ld+json">'
                '{"url":"https://example.com/another-plugin","softwareVersion":"1.2.2"}'
                "</script>"
            )
        )
